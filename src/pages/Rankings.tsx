            ...
            {ranking.map((r, i) => (
              <tr
                ...
                className={`border-b border-border/60 last:border-0 ${
                  i === 0
                    ? 'border-l-4 border-l-accent-yellow bg-accent-yellow/10'
                    : r.uid === user?.uid
                      ? 'bg-accent-blue/10'
                      : ''
                }`}
              >
                ...
                <td className="px-4 py-3">
                  <Link
                    to={`/perfil/${r.uid}`}
                    className={`hover:text-accent-blue hover:underline ${i === 0 ? 'font-semibold text-slate-50' : ''}`}
                  >
                    {r.nome}
                  </Link>
                </td>
                <td className={`px-4 py-3 font-semibold ${i === 0 ? 'text-accent-yellow' : 'text-accent-blue'}`}>
                  {r.wpm}
                </td>
  ...
  return (
    <button
      ...
      className={`rounded-lg px-4 py-2 text-sm font-medium transition-colors focus-ring ${
        active ? 'bg-card text-slate-50' : 'text-slate-400 hover:text-slate-200'
      }`}
    >
      {children}
    </button>
