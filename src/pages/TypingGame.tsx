...
export default function TypingGame() {
  ...
  const liveStats = useMemo(() => {
    ...
    const s = calculateStats(typed, currentPhase.texto, Math.max(elapsed, 0.1), errorCount);
    return { wpm: s.wpm, precisao: s.precisao };
  }, [typed, elapsed, errorCount, currentPhase.texto, startTime]);

  return (
    <main className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <div className="mb-6 flex items-center gap-2">
        <h1 className="font-display text-xl font-bold text-slate-50">{currentPhase.nome}</h1>
        <DifficultyBadge dificuldade={currentPhase.dificuldade} />
      </div>

      {fase === 'aguardando' && (
        <div className="glass rounded-2xl p-8 text-center shadow-soft">
          <p className="mb-6 text-slate-300">
            Você tem cerca de <span className="text-accent-blue">{currentPhase.tempoRecomendado}s</span> para
